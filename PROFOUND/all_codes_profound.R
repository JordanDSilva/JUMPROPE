...
star_mask = function(input_args){
  ...
  star_masker = function(ref, gaia, pro, psf){
    ...
    # star_rad_list = foreach(kk = 1, .combine = "c") %do% {
      ...
      rr = c(dr_vec[combined_flux > 0.8*depth & !is.na(combined_flux) & sn_tot > 1.0])
      ff = c(combined_flux[combined_flux > 0.8*depth & !is.na(combined_flux) & sn_tot > 1.0])
      ff_err = c(combined_flux_err[combined_flux > 0.8*depth & !is.na(combined_flux) & sn_tot > 1.0])
